...
module.exports = [
  ...
  {
    id: 'FOC-022',
    title: 'تحويل فائض العجز للغداء + دفعةٌ زائدة تُنشئ رصيدًا دائنًا (Overflow + Credit)',
    member: 'LAB-001',
    narrative: 'محمد أحمد (عليه 750) يُقبَض منه تحصيلُ ذمّةٍ موجَّهٌ للعجز بمبلغ 9000 (العجز المتبقّي 3000). يُسدَّد رصيدُه المستحقّ بالكامل أوّلًا (750 → 0)، ثمّ يُسجَّل الباقي 8250 رصيدًا دائنًا للعضو؛ ويُسدَّد العجز المشترك (0) ويتحوّل فائضه 6000 للغداء.',
    business: 'أمران دستوريّان في عمليةٍ واحدة. (أ) الفائض المشترك: المبلغ الموجَّه للعجز أكبر من المتبقّي، فأُغلق العجز المشترك كاملًا (−3000 → 0) وتحوّل الفائض 6000 تلقائيًّا للغداء بقاعدة قراءةٍ لا صفوف تُكتب (ق2 الاشتقاق، ق9 حدّ العجز). (ب) الدفعة الزائدة للعضو (قرار المالك — توضيح دستوريّ): المقبوض 9000 أكبر من المستحقّ 750، فأُغلق المستحقّ بالكامل أوّلًا (صار 0) ثمّ سُجِّل الباقي = المقبوض − المستحقّ = 8250 رصيدًا دائنًا للعضو (ذمّةً سالبة). الرصيد الدائن لا يضيع ولا يُعامَل خطأً ولا يبقى معلّقًا بدَينٍ مُغلَق — يبقى ملكًا للعضو ويشارك في التسوية عند توليد الاشتراكات القادمة. الديوان لم يتغيّر.',
    laws: ['1 حفظ القيمة', '2 الاشتقاق', '5 حفظ التاريخ', '9 حدّ العجز'],
    op: { type: 'receipt', fund: 'donation', payerType: 'member', member: 'LAB-001', amount: 9000, kind: 'cash', display: 'food', alloc: 'reduce_deficit' },
    expect: (b, a) => [
      { label: 'العجز المشترك −3000 → 0 (سُدّد بالكامل)', pass: eq(b.treasuries.defRem, -3000) && eq(a.treasuries.defRem, 0), detail: b.treasuries.defRem + ' → ' + a.treasuries.defRem },
      ...
      { label: 'المستحقّ على العضو أُغلق بالكامل أوّلًا (750 → 0 مستحقّ)', pass: eq(b.members['LAB-001'].finalBalance, 750) && Math.max(0, R2(a.members['LAB-001'].finalBalance)) === 0, detail: b.members['LAB-001'].finalBalance + ' → مستحقّ ' + Math.max(0, R2(a.members['LAB-001'].finalBalance)) },
      { label: 'رصيدٌ دائنٌ للعضو = المقبوض − المستحقّ (9000 − 750 = 8250)', pass: eq(a.members['LAB-001'].creditBalance, 8250) && eq(a.members['LAB-001'].creditBalance, 9000 - b.members['LAB-001'].finalBalance), detail: 'credit=' + a.members['LAB-001'].creditBalance },
      { label: 'الرصيد الدائن محفوظ (لم يضِع ولم يُعامَل خطأً)', pass: a.members['LAB-001'].creditBalance > 0 && eq(a.members['LAB-001'].finalBalance, -8250), detail: 'finalBalance=' + a.members['LAB-001'].finalBalance },
      { label: 'الديوان لم يتغيّر', pass: eq(a.treasuries.diwan, b.treasuries.diwan), detail: a.treasuries.diwan }
    ]
  },
  {
