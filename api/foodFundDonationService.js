...
async function computeFoodFundFigures(supabase, computeOperationalFoodBalance) {
  ...
  const currentBalance = round2(operational + supportCurrent);

  // remaining deficit = constant + reductions; capped at 0 (does not spill into current).
  const rawRemaining = round2(deficitConstant + reduceDeficit);
  const deficitRemaining = rawRemaining > 0 ? 0 : rawRemaining;
  const overReduction = rawRemaining > 0 ? rawRemaining : 0; // flagged, not auto-applied

  return {
    current_food_balance: currentBalance,
    historical_deficit_remaining: deficitRemaining,
    deficit_constant: deficitConstant,
    support_current_total: supportCurrent,
    reduce_deficit_total: reduceDeficit,
    over_reduction_flagged: overReduction, // > 0 means deficit fully cleared with surplus to review
  };
}
