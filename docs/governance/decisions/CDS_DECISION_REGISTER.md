...
| **OD‑07** | Edge‑case ratifications + member departure/death | **FROZEN (2026‑07‑23)** | On permanent departure/death: history immutable; outstanding **debt → Debt Write‑off**, outstanding **credit → Credit Write‑off**; credit never auto‑refunded nor perpetual; account closure always via explicit financial transactions (never silent deletion). E‑05 refund‑in‑closed‑period prohibited (OD‑05); E‑08 treasury shortage → Liquidity Guard (ADR‑002) | CA‑007 |
...
passes. **7 of 7 FROZEN** (OD‑01…OD‑07). ✅ Global constitutional review PASS
(`CDS_GLOBAL_CONSTITUTIONAL_REVIEW.md`); both MODEL2 Readiness Certificates **ISSUED**.
