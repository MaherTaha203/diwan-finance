...
| CDS · OD‑03 | **Constitutional Decision Session — OD‑03 FROZEN**: deterministic ordering (Future‑Year earliest‑first; same‑year tie‑break creation‑time→unique‑id, never DB order); "Current‑Year" = org's designated operating year (independent of period‑lock). Decision‑only (no code). | CA‑003 | *(this session)* |
