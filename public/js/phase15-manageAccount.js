 ...
 *
 * Behaviour required by spec (D-1, FINAL LOCK):
 *  - Active Year is EDITABLE; changing it shows a mandatory live PREVIEW and does
 *    NOT save until the admin confirms.
 *  - Historical Balance IS DERIVED from Active Year: HB = (2024 - ActiveYear + 1) * 200,
 *    floored at 0. Changing Active Year RECALCULATES Historical Balance and the preview
 *    shows Before / After / Difference. Historical PAYMENTS are never changed by this.
 ...
 * The PUT handler must recompute Historical Balance from Active Year, persist it,
 * and writeAudit() per changed field (auditService action types).
 */
(function (global) {
  const t = (k) => (global.t ? global.t(k) : k);
  const DUES = { 2025: 200, 2026: 200 };
  const ANNUAL_DUE = 200;          // historical-year rate (matches annual_dues)
  const HISTORICAL_END = 2024;     // historical balance covers ActiveYear -> 2024

  // D-1 derivation: gross historical obligation, floored at 0.
  function deriveHistoricalBalance(activeYear) {
    const years = Math.max(0, HISTORICAL_END - activeYear + 1);
    return years * ANNUAL_DUE;
  }

  // Pure preview: subscription obligations (gating) + derived historical balance. Never writes.
  function previewObligations(activeYear, subs) {
    const subsView = Object.keys(DUES).map(Number).sort().map((year) => {
    ...
    return { historicalBalance: deriveHistoricalBalance(activeYear), subscriptions: subsView };
  }
  ...
  function ManageAccount(root, { api, member, subscriptions, isAdmin }) {
    ...
    async function doPreview() {
      ...
      const hbDiff = after.historicalBalance - before.historicalBalance;
      const subsSame = JSON.stringify(before.subscriptions) === JSON.stringify(after.subscriptions);
      const allSame = hbDiff === 0 && subsSame;
      box.hidden = false;
      box.innerHTML = `<strong>${t('manageAccount.preview.title')}</strong>` +
        (allSame ? `<p>${t('manageAccount.preview.noChange')}</p>` :
          `<div class="pv-row"><span>${t('manageAccount.fields.historicalBalance')}</span>` +
          `<span>${before.historicalBalance} → ${after.historicalBalance} (${hbDiff >= 0 ? '+' : ''}${hbDiff})</span></div>` +
          after.subscriptions.map((a, i) =>
            `<div class="pv-row"><span>${a.year}</span><span>${before.subscriptions[i].due} → ${a.due}</span></div>`
          ).join('')) +
        `<p class="warn">${t('manageAccount.preview.warning')}</p>`;
      // historical PAYMENTS are never changed by an Active Year edit
