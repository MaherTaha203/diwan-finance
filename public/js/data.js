...
'id,name,phone,member_code,notes,opening_balance,prepaid_subscription_ils,is_active,created_at,active_from_year,historical_balance_ils,historical_payments_ils,credit_balance_ils,is_migration_exception'
      ...
      /* MODEL2 V2.0 (S2) — dedicated tables read by the balance models; empty until the
      ...
      /* IG-016 (FD-004) — immutable close-time report snapshots (few rows; latest per year wins). */
      SB.from('fiscal_snapshots').select('id,closed_through,previous_lock,snapshot,created_by,created_at').order('created_at',{ascending:false}),
    ...
    if(typeof window!=='undefined' && window.RECEIPT_ALLOCATION_ENABLED===true){
      try{ const _ar=await SB.from('allocation_records').select('id,source_ref,source_kind,member_id,obligation_kind,year,amount_allocated,voided_at,refunded_at').eq('source_kind','receipt_settlement'); DB.allocation_records=(_ar&&_ar.data)||[]; }catch(_){/* tolerant */}
    }
